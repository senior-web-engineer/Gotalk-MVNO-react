...
const getList = async (params) => {
    ...
    const response = await api.get(`v1/coupons${params_page}`);
...
const removePlan = async (params) => {
    const response = await api.delete(`v1/coupons/${params.id}`);

    return { data: response.data.payload };
};

const create = async (params) => {
    const response = await api.post(`v1/coupons`, params.data);

    return { data: response.data.payload };
};

const update = async (params) => {
    await api.put(`v1/coupons/${params.data.id}`, params.data);
